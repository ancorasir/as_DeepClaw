...
def tf_reader(tf_record_filename_queue):
    ...
    #                 ) #[2,1]
    label_ = tf.stack([label, tf.subtract(1.0, label)], axis=0)
    labels = tf.tile(tf.reshape(label_, [1, 2]),
                     [images.get_shape()[0].value, 1]
                     )
...
def inputs(filenames, batch_size, num_epochs):
	...
	images, motions, labels = tf_reader(filename_queue)
        ...
        images_batch, motions_batch, labels_batch = tf.train.batch([images, motions, labels], batch_size=batch_size, capacity=50 + 3 * batch_size)
	actual_batch_size = images_batch.get_shape()[0].value * images_batch.get_shape()[1].value
        return tf.reshape(images_batch, [actual_batch_size, 472*2, 472, 3]), tf.reshape(motions_batch, [actual_batch_size, 3]), tf.reshape(labels_batch, [actual_batch_size, 2])
