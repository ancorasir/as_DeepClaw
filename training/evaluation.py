	...
	#total_accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
        ...
        try:
            while not coord.should_stop():
                y_value, labels_value = sess.run([y, labels_batch])
		...
		step += 1
            ...
            correct_prediction = np.argmax(Y_value[1:,:], 1) == np.argmax(Label_value[1:,:], 1)
            total_accuracy = np.mean(correct_prediction.astype(float))
            print('total_accuracy = %.3f'%total_accuracy)
            coord.request_stop()
