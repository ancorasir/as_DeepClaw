...
# Construct model
def inference(images, motions, is_training,keep_prob):
    ...
    with tf.variable_scope('conv1') as scope:
        weights = tf.get_variable('weights', [6, 6, 3, 64], initializer=tf.truncated_normal_initializer(stddev = 0.01, dtype=tf.float32), regularizer=regularizer)
    ...
    with tf.variable_scope('conv2') as scope:
        weights = tf.get_variable('weights', [5, 5, 64, 64], initializer=tf.truncated_normal_initializer(stddev = 0.01, dtype=tf.float32), regularizer=regularizer)
    ...
    with tf.variable_scope('fc1'):
        ...
        weights = tf.get_variable('weights', [3, 64], initializer=tf.truncated_normal_initializer(stddev = 0.01, dtype=tf.float32), regularizer = regularizer)
    ...
    with tf.variable_scope('conv8') as scope:
        weights = tf.get_variable('weights', [3, 3, 64, 64], initializer=tf.truncated_normal_initializer(stddev = 0.01, dtype=tf.float32), regularizer=regularizer)
    ...
    with tf.variable_scope('fc2'):
        weights = tf.get_variable('weights', [27*14*64, 64], initializer=tf.truncated_normal_initializer(stddev = 0.01, dtype=tf.float32), regularizer=regularizer)
        biases = tf.get_variable('biases', [64], initializer=tf.constant_initializer(0.1))
        fc2 = tf.nn.relu(tf.matmul(conv16_flat, weights) + biases)
    with tf.variable_scope('fc3'):
        weights = tf.get_variable('weights', [64, 64], initializer=tf.truncated_normal_initializer(stddev = 0.01, dtype=tf.float32), regularizer=regularizer)
        biases = tf.get_variable('biases', [64], initializer=tf.constant_initializer(0.1))
        fc3 = tf.nn.relu(tf.matmul(fc2, weights) + biases)
    #dropout with the probility kept 'keep_prob'
    ...
    with tf.variable_scope('out'):
        weights = tf.get_variable('weights', [64, 2], initializer=tf.truncated_normal_initializer(stddev = 0.01, dtype=tf.float32), regularizer=regularizer)
        biases = tf.get_variable('biases', [2], initializer=tf.constant_initializer(0.1))
        logits = tf.matmul(fc3_drop, weights) + biases
    return logits
