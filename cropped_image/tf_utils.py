...
def inputs(filenames, batch_size, num_epochs, is_train=1):
	...
	if is_train == 1:
        ...
        return I, T*1000, tf.one_hot(tf.to_int32(tf.reshape(L,[-1])), NUM_CLASSES)

        #T_index = tf.floordiv(tf.reshape(T,[-1]), 2*3.14/NUM_THETAS)+1
        #return I, tf.reshape(L,[-1,1]), tf.one_hot(tf.to_int32(T_index), NUM_CLASSES)
