...
def cut_img(img_name):
    ...
    up = 440
    shift = 360
    box = (lft, up, lft+shift, up+shift)
    
    img = img.crop(box)
    img.save(img_name.split('.')[0] + '_cut.jpg')
    return img

def compare_image(imga_name,imgb_name):
    imga = Image.open(imga_name)
    imgb = Image.open(imgb_name)
...
if __name__ == '__main__':
    ...
    grasp_num = 0
    for i in range(1,40):
        imga = 'cam1_I_'+str(i)+'_13_color_cut.jpg'
        imgb = 'cam1_I_'+str(i+1)+'_13_color_cut.jpg'
